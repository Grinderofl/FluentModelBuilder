...
{
    public class AlterationCollectionBase<T, TAlteration> : IEnumerable<TAlteration>
        where T : AlterationCollectionBase<T, TAlteration>
    {
        ...
        public T Add(TAlteration alteration)
        {
            if (
                !Alterations.Exists(
                    a =>
                        a.GetType() == alteration.GetType() &&
                        alteration.GetType() != typeof (EntityTypeOverrideAlteration)))
