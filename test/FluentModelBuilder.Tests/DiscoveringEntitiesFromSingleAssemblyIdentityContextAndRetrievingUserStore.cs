...
namespace FluentModelBuilder.Tests
{
    public class DiscoveringEntitiesFromSingleAssemblyIdentityContextAndRetrievingUserStore : IClassFixture<ModelFixture>
    {
        ...
        {
            Assert.True(Model.EntityTypes.Any(x => x.ClrType == typeof(EntityOne)));
            Assert.True(Model.EntityTypes.Any(x => x.ClrType == typeof(EntityTwo)));
        }

        [Fact]
        public void AddsAllEntities()
        {
            Assert.Equal(8, Model.EntityTypes.Count);
        }
